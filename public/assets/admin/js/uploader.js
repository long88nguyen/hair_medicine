...
var uploader = {
    ...
    onchangeUpload: function () {
        //console.log($("#" + this.inputImageID).parent().find('.crop-sizes').val()); return false;
        //this.params.cropSizes = (this.isTinymce) ? '' : document.getElementById('crop-sizes').value;
        //this.params.ratio = (this.isTinymce) ? false : document.getElementById("crop-ratio").value;
        //this.params.useOriginal = (this.isTinymce) ? false : document.getElementById("use-original").checked;
        //turnOnOverlay();
        this.params.cropSizes = $("#" + this.inputImageID).parent().find('.crop-sizes').val();
    ...
    uploadMultiple: function () {
        ...
        this.params.ratio = document.getElementById("crop-ratio").value;
    ...
    success: function (response) {
        ...
        if (response.status == true) {
            ...
            if (this.type == 'media') {
                if (this.isTinymce) {
                    ...
                    if ((caption != null)) {
                        ...
                        insertHTML += '     <'+ response.type +' width="500" height="320" class="mw-100" controls><source src="' + response.path + '" type="'+ sourceType +'"></'+ response.type +'>';
                ...
                if (this.isTinymce) {
                    ...
                    if ((caption != null)) {
                        ...
                        insertHTML += '     <img class="img-responsive" src="' + settings.cdnUrl + response.images[0] + '" alt="' + caption + '"/>';
                        insertHTML += '     <figcaption class="d-block text-center bg-light text-primary">' + caption + '</figcaption>';
                        insertHTML += ' </figure>';
                        insertHTML += '</div><p></p>';
                    } else {
                    ...
                    if (this.params.useOriginal) {
                        ...
                        url = (typeof (response.medium_images) == 'undefined') ? (settings.cdnUrl + response.images[0]) : response.medium_images[0];
                    }
            ...
            if (typeof(response.errors) != 'undefined') {
                Swal.fire('<small class="text-danger">' + response.errors[0] + '</small>');
            }
        }
